...
public class gameController : MonoBehaviour {
	...
	void Start () {
		...
		//intitilizes positions of cubes
	...
	public static void moveAirplane(){
		if (airplaneY > movedPositionY) {
			//makes the last cube the airplane cube was on white or black if it was the dropOffCube
	...
	public static void processClick(GameObject clickedCube, int x, int y) {
		...
		//makes movedPosition the cube we clicked on
	...
	void Update (){
		//updates only every turn
		if (Time.time > turnTime) {
			//resets the scoreText to blank every turn so it only pops up for a short time
			scoreText.text = " ";
			//moves the airplane
			if (airplaneActivated == true) {
				moveAirplane ();
		...
		if (airplaneX == 0 && airplaneY == 0) {
			...
			//checks if airplane is in cargo drop off area, drops off cargo if so and adds score
				...
				if (cargo > minCargo) {
					...
					//score text pops up
					scoreText.text = "+" + cargo + "!";
					cargo = 0;
					cargoAirplaneText.text = "Cargo: " + cargo;
				}
			} else {
				//resets turnTime so cargo won't be added instantly back at the port, only once per turn
				turnTime = Time.time;
			}
			//increases turnTime so turns remain timeSpace seconds
			turnTime += timeSpace;
			}
		}
	}